...
public abstract class Database {
	 ...
	 * 
	 * @param commonPlugin Plugin instancing this database
	 */
	public Database(CommonPlugin commonPlugin) {
		// Store the CommonPlugin
		...
		try {
			...
			Method method = CommonPlugin.class.getDeclaredMethod("getClassLoader");
			method.setAccessible(true);

			// Store the ClassLoader
			this.classLoader = (ClassLoader) method.invoke(commonPlugin);
		} catch (Exception ex) {
	...
	private String replaceDatabaseString(String input) {
		input = input.replaceAll("\\{DIR\\}", commonPlugin.getDataFolder().getPath().replaceAll("\\\\", "/") + "/");
		input = input.replaceAll("\\{NAME\\}", commonPlugin.getDescription().getName().replaceAll("[^\\w_-]", ""));
