...
import com.alta189.chavabot.plugin.exceptions.InvalidDescriptionFileException;
import com.alta189.chavabot.plugin.exceptions.InvalidPluginException;
import com.alta189.chavabot.plugin.exceptions.UnknownDependencyException;

public class CommonPluginManager implements PluginManager {

	private final ChavaManager chavamanager;
	...
	public CommonPluginManager(final ChavaManager chavamanager) {
	...
	public void registerPluginLoader(Class<? extends PluginLoader> loader) {
		...
		try {
			Constructor<? extends PluginLoader> constructor = loader.getConstructor(new Class[] {ChavaManager.class});

			instance = constructor.newInstance(new Object[] {chavamanager});
		} catch (Exception e) {
		...
		if (updateDir != null && updateDir.isDirectory()) {
			...
			if (update.exists() && update.isFile()) {
				try {
					...
					chavamanager.getLogger().log(Level.SEVERE, new StringBuilder().append("Error copying file '").append(update.getPath()).append("' to its new destination at '").append(paramFile.getPath()).append("': ").append(e.getMessage()).toString(), e);
				}
				update.delete();
			}
		}
		...
		while(!failed || lastPass) {
			...
			while(iterator.hasNext()) {
				...
				try {
					...
					if (lastPass) {
						chavamanager.getLogger().log(Level.SEVERE, new StringBuilder().append("Unable to load '").append(file.getName()).append("' in directory '").append(paramFile.getPath()).append("': ").append(e.getMessage()).toString(), e);
						iterator.remove();
					} else {
						plugin = null;
					}
	...
	public void enablePlugin(Plugin plugin) {
		if (!plugin.isEnabled()) {
			...
			try {
				...
				chavamanager.getLogger().log(Level.SEVERE, new StringBuilder().append("An error ocurred in the Plugin Loader while enabling plugin '").append(plugin.getDescription().getFullName()).append("': ").append(e.getMessage()).toString(), e);
			}
		}
	}
