...
import com.alta189.chavabot.plugin.exceptions.UnknownSoftDependencyException;
...
public class CommonPluginLoader implements PluginLoader {
	...
	public CommonPluginLoader(final ChavaManager chavamanager) {
	...
	public void enablePlugin(Plugin paramPlugin) {
		...
		if (!paramPlugin.isEnabled()) {
			...
			try {
				...
				chavamanager.getLogger().log(Level.SEVERE, new StringBuilder().append("An error occured when enabling '").append(paramPlugin.getDescription().getFullName()).append("': ").append(e.getMessage()).toString(), e);
			}
	...
	public void disablePlugin(Plugin paramPlugin) {
		...
		if (paramPlugin.isEnabled()) {
			...
			try {
				...
				chavamanager.getLogger().log(Level.SEVERE, new StringBuilder().append("An error occured when disabling plugin '").append(paramPlugin.getDescription().getFullName()).append("' : ").append(e.getMessage()).toString(), e);
			}
	...
	public Plugin loadPlugin(File paramFile, boolean ignoresoftdepends) throws InvalidPluginException, InvalidPluginException, UnknownDependencyException, InvalidDescriptionFileException {
		...
		try {
			...
			result.initialize(this, chavamanager, desc, dataFolder, paramFile, loader);
		} catch (Exception e) {
			throw new InvalidPluginException(e);
		}
